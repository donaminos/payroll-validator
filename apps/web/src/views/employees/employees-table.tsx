...
async function fetchEmployees({ page = 1, limit = 50, search = '' }: { page?: number; limit?: number; search?: string; }): Promise<EmployeesResponse> {
...
function getStatusBadge(status: Employee['status']) {
  const variants: Record<Employee['status'], 'default' | 'secondary' | 'outline'> = {
    active: 'default',
    inactive: 'secondary',
    on_leave: 'outline',
  };
...
function getContractTypeBadge(contractType: Employee['contractType']) {
  const variants: Record<Employee['contractType'], 'default' | 'secondary' | 'outline'> = {
    CDI: 'default',
    CDD: 'secondary',
    Interim: 'outline',
    Stage: 'outline',
  ...
  return (
    <Badge variant={variants[contractType]}>
      {contractType}
    </Badge>
  );
}
...
export async function EmployeesTable({ searchParams }: EmployeesTableProps) {
  ...
  // Pagination controls
  const { page: currentPage, totalPages, hasNextPage, hasPreviousPage } = employeesData.pagination;
    ...
    <Card>
      ...
      <CardContent>
        ...
        <form className="mb-4 flex gap-2" method="get">
          ...
          />
          <button type="submit" className="px-4 py-2 rounded bg-primary text-white">Rechercher</button>
        </form>
        <DataTable
        ...
        <div className="flex justify-center gap-2 mt-6">
          {hasPreviousPage && (
            <Link href={getPageHref(currentPage - 1)} className="px-3 py-1 border rounded">Précédent</Link>
          )}
          <span className="px-3 py-1">Page {currentPage} / {totalPages}</span>
          {hasNextPage && (
            <Link href={getPageHref(currentPage + 1)} className="px-3 py-1 border rounded">Suivant</Link>
          )}
        </div>
      </CardContent>
    </Card>
